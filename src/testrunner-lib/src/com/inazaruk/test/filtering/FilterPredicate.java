...
import com.inazaruk.test.predicate.HasAnnotation;
 ...
 * 
 * For any TestMethod to be included it should have ANY of "+" annotations and NONE of the "-" annotations.
...
public class FilterPredicate implements Predicate<TestMethod>
{
	...
	private Predicate<TestMethod> createPredicate(List<FilterEntry> filterEntries)
	{
		...
		List<Predicate<TestMethod>> orPredicates = 
						new ArrayList<Predicate<TestMethod>>(filterEntries.size());
			
		List<Predicate<TestMethod>> andPredicates = 
						new ArrayList<Predicate<TestMethod>>(filterEntries.size());
		...
		if(orPredicates.isEmpty()) orPredicates.add(TRUE_PREDICATE);
		if(andPredicates.isEmpty()) andPredicates.add(TRUE_PREDICATE);		
		return Predicates.and(Predicates.or(orPredicates), Predicates.and(andPredicates));
	}
	
	private void addPredicate(int action, 
